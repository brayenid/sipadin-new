...
export default function RekapKehadiranView({
  ...
  const [isCetakLengkapOpen, setIsCetakLengkapOpen] = useState(false);
  ...
  return (
    <div className="space-y-6 pb-20 lg:pb-0">
            ...
            <CardHeader className="px-4 py-4 sm:px-6 sm:py-5 border-b border-slate-100 bg-slate-50/40">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <div className="hidden sm:flex items-center gap-2">
                  <DropdownMenu>
                    <DropdownMenuTrigger
                      className="inline-flex items-center justify-center rounded-md text-xs font-semibold h-9 px-3 gap-1.5 bg-indigo-600 hover:bg-indigo-700 text-white shadow-xs cursor-pointer transition-colors focus:outline-hidden"
                    >
                      <FileText className="w-3.5 h-3.5" />
                      <span>Cetak & Ekspor Laporan</span>
                      <ChevronDown className="w-3.5 h-3.5 opacity-80" />
                    </DropdownMenuTrigger>
                    <DropdownMenuContent align="end" className="w-64 p-1.5 shadow-lg rounded-xl border-slate-200">
                      <DropdownMenuItem
                        onClick={() => setIsCetakLengkapOpen(true)}
                        className="flex items-start gap-2.5 p-2 text-xs font-medium cursor-pointer rounded-lg text-slate-700 hover:bg-indigo-50 hover:text-indigo-900 focus:bg-indigo-50 focus:text-indigo-900"
                      >
                        <div className="w-7 h-7 rounded-md bg-indigo-100 text-indigo-700 flex items-center justify-center shrink-0 mt-0.5">
                          <FileText className="w-3.5 h-3.5" />
                        </div>
                        <div className="flex-1">
                          <p className="font-semibold text-slate-900 leading-snug">Laporan Mode Lengkap</p>
                          <p className="text-[10px] text-slate-500 mt-0.5 leading-tight">Booklet kompilasi seluruh kegiatan & presensi</p>
                        </div>
                      </DropdownMenuItem>

                      <DropdownMenuItem
                        onClick={() => setIsCetakPdfOpen(true)}
                        className="flex items-start gap-2.5 p-2 text-xs font-medium cursor-pointer rounded-lg text-slate-700 hover:bg-red-50 hover:text-red-900 focus:bg-red-50 focus:text-red-900 mt-0.5"
                      >
                        <div className="w-7 h-7 rounded-md bg-red-100 text-red-700 flex items-center justify-center shrink-0 mt-0.5">
                          <FileText className="w-3.5 h-3.5" />
                        </div>
                        <div className="flex-1">
                          <p className="font-semibold text-slate-900 leading-snug">Ringkasan Rekap PDF</p>
                          <p className="text-[10px] text-slate-500 mt-0.5 leading-tight">Laporan tabel & grafik rangking OPD</p>
                        </div>
                      </DropdownMenuItem>

                      <DropdownMenuItem
                        onClick={handleExportExcelOpd}
                        className="flex items-start gap-2.5 p-2 text-xs font-medium cursor-pointer rounded-lg text-slate-700 hover:bg-emerald-50 hover:text-emerald-900 focus:bg-emerald-50 focus:text-emerald-900 mt-0.5"
                      >
                        <div className="w-7 h-7 rounded-md bg-emerald-100 text-emerald-700 flex items-center justify-center shrink-0 mt-0.5">
                          <FileSpreadsheet className="w-3.5 h-3.5" />
                        </div>
                        <div className="flex-1">
                          <p className="font-semibold text-slate-900 leading-snug">Ekspor ke Excel (.xlsx)</p>
                          <p className="text-[10px] text-slate-500 mt-0.5 leading-tight">Unduh spreadsheet rekapitulasi OPD</p>
                        </div>
                      </DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-64 p-1.5 shadow-lg rounded-xl border-slate-200">
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <p className="font-semibold text-slate-900 leading-snug">Ringkasan Pegawai PDF</p>
                          <p className="text-[10px] text-slate-500 mt-0.5 leading-tight">Laporan kepatuhan presensi pegawai</p>
                        </div>
                      </DropdownMenuItem>
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <p className="font-semibold text-slate-900 leading-snug">Ekspor ke Excel (.xlsx)</p>
                          <p className="text-[10px] text-slate-500 mt-0.5 leading-tight">Unduh spreadsheet rekapitulasi pegawai</p>
                        </div>
                      </DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
      ...
      <CetakRekapLengkapModal
        isOpen={isCetakLengkapOpen}
        onClose={() => setIsCetakLengkapOpen(false)}
        periodeLabel={periodeLabelText}
        filters={{
          startDate: customStartDate || (selectedBulan !== "ALL" ? `${selectedYear}-${parseInt(selectedBulan, 10) < 10 ? `0${selectedBulan}` : selectedBulan}-01` : `${selectedYear}-01-01`),
          endDate: customEndDate || (selectedBulan !== "ALL" ? `${selectedYear}-${parseInt(selectedBulan, 10) < 10 ? `0${selectedBulan}` : selectedBulan}-${new Date(parseInt(selectedYear, 10), parseInt(selectedBulan, 10), 0).getDate()}` : `${selectedYear}-12-31`),
