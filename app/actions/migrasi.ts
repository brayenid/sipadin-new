...
export async function executeImportMigration(payload: {
  ...
  try {
      ...
      for (const p of jsonData.pegawais || []) {
        ...
        if (resolution && resolution.action === "USE_EXISTING" && resolution.v2Id) {
          ...
          if (existingPeg) {
            ...
            logs.push(
              `Pegawai [${p.nama}] terdeteksi memiliki NIP sama [${cleanNip}] di target Team, otomatis dipetakan ke ID V2: ${existingPeg.id}`
            );
          } else {
    ...
    return {
      ...
      errorCount: 0,
      logs: [
        "❌ PROSES MIGRASI GAGAL (DI-ROLLBACK):",
        `Penyebab: ${err.message || String(err)}`,
        err.stack ? `Detail: ${err.stack.split("\n").slice(0, 3).join("\n")}` : "",
      ],
