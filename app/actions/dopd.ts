...
export async function saveDopdTransaction(
  ...
  try {
    ...
    return { success: false, error: error.message || "Gagal menyimpan DOPD." };
  }
}

export async function saveDopdHonorarium(
  spjId: string,
  dopdItems: any[],
  dopdMeta?: any
): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const meta =
        spj.metaDokumen && typeof spj.metaDokumen === "object"
          ? (spj.metaDokumen as any)
          : {};

      if (meta.daftarTandaTerima && meta.daftarTandaTerima.tandaTerimaData) {
        const narasumberData = Object.values(
          meta.daftarTandaTerima.tandaTerimaData
        ) as any[];
